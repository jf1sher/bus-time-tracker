...
import logging
from waveshare_epd import epd5in83b_HD
import time
...
try:
    logging.info("epd5in83b_HD Demo")
    
    epd = epd5in83b_HD.EPD()
    ...
    HBlackimage = Image.open(os.path.join(picdir, '5in83b_HD_b.bmp'))
    HRYimage = Image.open(os.path.join(picdir, '5in83b_HD_r.bmp'))
    epd.display(epd.getbuffer(HBlackimage), epd.getbuffer(HRYimage))
    time.sleep(2)
    ...
    logging.info("ctrl + c:")
    epd5in83b_HD.epdconfig.module_exit()
    exit()
